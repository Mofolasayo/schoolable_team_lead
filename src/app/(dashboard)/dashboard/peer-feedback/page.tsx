...
// Rating criteria for regular colleagues
const PEER_RATING_ITEMS = [
    {
    ...
    {
        key: "collaborationRating" as const,
        name: "Collaboration",
    ...
    {
        key: "adaptabilityRating" as const,
        name: "Adaptability",
    ...
    {
        key: "accountabilityRating" as const,
        name: "Accountability",
...
// Additional criteria for rating team leads
const LEADERSHIP_RATING_ITEMS = [
    {
        key: "orgGuidanceRating" as const,
    ...
    {
        key: "peopleCultureRating" as const,
    ...
    return (
        <div className="flex items-center gap-1.5">
            {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                    <Star
                        className={`w-6 h-6 transition-all ${star <= (hovered ?? value)
...
export default function PeerFeedbackPage() {
    ...
    const [feedbackList, setFeedbackList] = useState<Record<string, PeerFeedback>>({});
    ...
    const completedCount = Object.values(feedbackList).filter((f) => {
        ...
        return ratings.supportRating > 0 &&
            ratings.collaborationRating > 0 &&
            ratings.adaptabilityRating > 0 &&
            ratings.valuesRating > 0 &&
            ratings.accountabilityRating > 0 &&
    ...
    const updateNotes = (field: "strengths" | "areasForImprovement", value: string) => {
        if (!currentColleague) return;
        const id = currentColleague.id;
        const existing = feedbackList[id];
        if (!existing) return;
    ...
    const handleSubmitAll = async () => {
        if (!allFeedbackComplete) {
            toast.error("Please complete feedback for all colleagues before submitting.");
            return;
        }
    ...
    const currentStep = showSummary ? colleagues.length + 1 : currentColleagueIndex + 1;
                ...
                <div className="max-w-3xl mx-auto space-y-8">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Review & Submit Feedback</h1>
                    ...
                    <Card className="bg-white border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <CardHeader className="pb-4">
                            <CardTitle className="text-lg font-semibold text-slate-900">Feedback Summary</CardTitle>
                            <CardDescription>Review your peer feedback before final submission.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-1">
                                {colleagues.map((colleague) => {
                                    ...
                                    return (
                                        <div key={colleague.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-colors">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="text-sm font-medium text-slate-700">{colleague.name}</span>
                                                    {colleague.isTeamLead && (
                                                        <Badge variant="outline" className="ml-2 text-xs">Team Lead</Badge>
                                                    )}
                                                </div>
                                            </div>
                                            {isComplete ? (
                    ...
                    <Card className="bg-gradient-to-br from-indigo-50 to-purple-50 border-indigo-100">
                        <CardContent className="p-6">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-slate-900">Your feedback is anonymous</h3>
                                    <p className="text-sm text-slate-600 mt-1">
                                        All peer feedback is submitted anonymously. Your colleagues will only see aggregated scores and comments.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-slate-200">
                        <Button variant="outline" onClick={goToPreviousColleague} className="bg-white">
        ...
        <div className="min-h-screen bg-slate-50/50 p-8">
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl font-bold text-slate-900">Weekly Peer Feedback</h1>
                ...
                <Card className="border-slate-200 shadow-sm bg-white overflow-hidden">
                    <div className={`h-24 relative ${currentColleague?.isTeamLead ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-gradient-to-r from-indigo-500 to-violet-500'}`}>
                        <div className="absolute -bottom-8 left-8">
                            <Avatar className="h-20 w-20 border-4 border-white shadow-md">
                                <AvatarImage src={currentColleague?.avatarUrl} />
                                <AvatarFallback className="text-xl bg-slate-100">{currentColleague?.name[0]}</AvatarFallback>
                            </Avatar>
                        </div>
                    </div>
                    <CardContent className="pt-10 px-8 pb-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <h2 className="text-2xl font-bold text-slate-900">{currentColleague?.name}</h2>
                                <p className="text-slate-500 font-medium">{currentColleague?.role} • {currentColleague?.department}</p>
                            </div>
                ...
                <div className="grid gap-4">
                    {PEER_RATING_ITEMS.map((item) => {
                        const Icon = item.icon;
                        const rating = currentFeedback.ratings[item.key];

                        return (
                            <Card key={item.key} className="border-slate-200 shadow-sm transition-all hover:shadow-md hover:border-indigo-100">
                                <CardContent className="p-5">
                                    <div className="flex gap-4">
                                        <div className={`p-2.5 h-10 w-10 rounded-xl ${item.bgColor} ${item.color} flex items-center justify-center shrink-0`}>
                    ...
                    {currentColleague?.isTeamLead && (
                        <>
                            <div className="pt-4 pb-2">
                                <h3 className="text-sm font-semibold text-slate-700 uppercase tracking-wide">Leadership Qualities</h3>
                            </div>
                            {LEADERSHIP_RATING_ITEMS.map((item) => {
                                const Icon = item.icon;
                                const rating = currentFeedback.ratings[item.key] || 0;

                                return (
                                    <Card key={item.key} className="border-slate-200 shadow-sm transition-all hover:shadow-md hover:border-amber-100">
                                        <CardContent className="p-5">
                                            <div className="flex gap-4">
                                                <div className={`p-2.5 h-10 w-10 rounded-xl ${item.bgColor} ${item.color} flex items-center justify-center shrink-0`}>
                                                    <Icon className="w-5 h-5" />
                ...
                <Card className="border-slate-200 shadow-sm">
                    <CardHeader className="pb-2 border-b border-slate-50">
                        <CardTitle className="text-base font-semibold text-slate-900">Additional Comments</CardTitle>
                        <CardDescription className="text-xs">Optional - Help your colleague grow with constructive feedback</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4 pt-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">What they do well</label>
                            <textarea
                                ...
                                className="w-full h-20 bg-slate-50 border-slate-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none resize-none transition-all placeholder:text-slate-400"
                            />
                        </div>
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                onChange={(e) => updateNotes("areasForImprovement", e.target.value)}
