...
export async function login(formData: FormData) {
  ...
  try {
    ...
    if (!profile?.is_team_lead && profile?.role !== 'admin') {
      if (profile?.team_lead_request_status === 'pending') {
        throw new Error('Your team lead request is pending approval.');
      }
    ...
    cookieStore.set(
      ...
      JSON.stringify({
        ...
        teamLeadRequestStatus: profile.team_lead_request_status || 'none',
